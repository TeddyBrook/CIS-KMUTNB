...
function View_obe3() {
    ...
    useEffect(() => {
        fetch('http://localhost:8081/api/pdfobe3')
            .then(response => response.json())
            .then(data => {
                setDocuments(data)
                console.log(data);
            })
            .catch(error => console.error('Error fetching PDF Outcome-based Education 3 :', error));
    }, []);
            ...
            <div className="document-container">
                ...
                <div className="document-bar">
                    ...
                    <div className="title-obe">
                        <button className="name-obe"> OBE 3 </button>
                        {filteredDocuments.map(doc => (
                            <div key={doc.pdf_Id} className="document-item">
                                <p> {doc.fileName} </p>
